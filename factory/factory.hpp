//
// Created by macie on 26.01.2023.
//
...
public:
    ...
    iterator find_by_id(ElementId id_) {auto it = std::find_if(Nodes.begin(), Nodes.end(), [id_](const auto& elem){ return (elem.get_id() == id_);}); return it;}
    const_iterator find_by_id(ElementId id_) const {auto it = std::find_if(Nodes.begin(), Nodes.end(),[id_](const auto& elem){return (elem.get_id() == id_);}); return it;}
